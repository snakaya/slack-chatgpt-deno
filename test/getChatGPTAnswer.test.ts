...
const chatGPTMessages: ChatGPTMessages[] = [{"role": "user", "content": "Is 3 a prime number?"}];
...
mod.describe("getChatGPTAnswer Normal", () => {
  mod.it("Normal Request", async () => {
		...
		try {
			const testAnswer = await getChatGPTAnswer.GetChatGPTAnswer(chatGPTMessages, openai_api_key);
